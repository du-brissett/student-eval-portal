...
public partial class SystemAdmin_AssignPositionWeight : System.Web.UI.Page
{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        else if (total == 100 && err == 0)
        {
            ...
            {
                BusinessLogic.UpdatePositionFacultyRole(lbPosition.SelectedValue, row["Role"].ToString(), Convert.ToInt32(row["Weight"]));
            }
            int index = lbPosition.SelectedIndex;
            lbPosition.Items.RemoveAt(index);
            if (lbPosition.Items.Count > 0)
                ClientScript.RegisterStartupScript(this.GetType(), "Alert", "alert('Insert Successful!');", true);
            else
                ClientScript.RegisterStartupScript(this.GetType(), "Alert", "alert('Insert Successful! All Positions have been assigned their role weightings.');", true);
            Reset();
        }
        
    }

    protected void lbPosition_SelectedIndexChanged(object sender, EventArgs e)
    {
